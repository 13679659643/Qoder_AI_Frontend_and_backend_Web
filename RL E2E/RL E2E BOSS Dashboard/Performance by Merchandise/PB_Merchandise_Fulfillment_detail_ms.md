...
### 4.10 IsMerchandiseRowVisible（辅助度量 — 矩阵行维度视觉对象级别筛选器）
...
> **关键操作**：在矩阵（主表）的"筛选器"窗格中，将 `[IsMerchandiseRowVisible]` 拖入"此视觉对象上的筛选器"，设置条件为 `为 1`（is 1）。这样 Request Order 当期没值的行整体不展示——四个行维度字段（brand / product_type / category_summary / category）所在的行隐藏，父行没值时其子行一并不展示。
...
| 9    | IsMerchandiseRowVisible                          | Visual Filters | 矩阵行维度视觉对象级别筛选器（Request Order 当期没值 → 0 隐藏行） |
...
| **值** | [Fulfillment PB Merchandise Cell Display]                                  |
| **筛选器** | [IsMerchandiseRowVisible] = 1（此视觉对象上的筛选器，Request Order 当期没值的行不展示） |
