...
假设平台、日期等筛选相同，数据如下；其他下级字段保持一致：
framework	    brand	本期    Cost	LP Cost
Foundation	    品牌    A	    60	    30
Foundation	    品牌    B	    无记录	 30
Acceleration	品牌    C	    40	    40
全部合计		                100	     100
当前矩阵只展示 framework，尚未展开到 brand。

### A：限定本期存在的完整行组合

先确定本期存在的：framework → brand → category → channel → mix_msg_Label_01再携带这些组合去计算 LP。本例只有“Foundation＋品牌 A”和“Acceleration＋品牌 C”符合条件。品牌 B 即使藏在折叠层级里，也不计入 LP。
framework	    本期Cost%	    LP Cost%	    Cost% VS LP
Foundation	    60/100＝60%	    30/70＝42.9%	+40.0%
Acceleration	40/100＝40%	    40/70＝57.1%	-30.0%
总计	        100%	        100%	        0.0%
特点：
本期确定比较集合，LP 分子、分母都限定在这个集合内。
矩阵展开、折叠不改变比较集合。
即使同一品牌两期都有数据，某个下级渠道或明细组合仅 LP 存在，该组合仍被排除。

### B：只限定当前展示层级

当前展示 framework，只检查该 framework 本期是否存在。Foundation 本期存在，所以 LP 计算 Foundation 下的全部历史数据，包括仅 LP 有数据的品牌 B。
framework	    本期 Cost%	    LP Cost%	        Cost% VS LP
Foundation	    60%	            (30＋30)/100＝60%	0.0%
Acceleration	40%	            40/100＝40%	        0.0%
总计	        100%	        100%	            0.0%
如果展开到 brand，再按本期存在的 framework＋brand 判断，品牌 B 就会被排除。特点：
比较的是“当前层级分类”的两期表现。
保留该分类下仅 LP 存在的下级数据。
若随展示层级重新确定集合，展开后 LP 范围及相关汇总值可能变化。

### 怎么选？

A：只拿本期实际存在的这批完整组合，回看它们上期的表现
B：看 Foundation，就比较 Foundation 两期全部表现，不关心内部品牌是否变化
常规经营看板更接近 B：保留当前分类筛选，只切换日期。 A 则属于特定的“本期成员回溯分析”，不建议默认使用。

## 3、日期比较的陷阱

### DAX 与 SQL 对空值的比较规则不同

#### 在 DAX 中：

BLANK() <= DATE(2026, 5, 23)   // 返回 TRUE
BLANK() 参与这种日期比较时，会按日期零值（1899-12-30）处理。因此：
'a03_e2e_customer_data_m'[register_date] <= __EndPeriodDate
当截止日是 2026-05-23 时，会放行：
注册日期不晚于截止日的记录；
注册日期为 BLANK() 的记录。
这是比较时的隐式处理，并不是实际把空日期填成了 1899-12-30。 依据：Microsoft DAX 比较运算规则（https://learn.microsoft.com/en-us/dax/dax-operator-reference#comparison-operators）。
除 == 之外的所有比较运算符都将 BLANK 视为等于数字 0、空字符串 ""、日期（1899年12月30日）或 FALSE。因此，当 [Column] 的值为 0 或 BLANK 时，[Column] = 0 会返回 true。相比之下，[Column] == 0 只有在 [Column] 的值为 0 时才为 true。

#### SQL 中：

NULL <= '2026-05-23'
结果是 UNKNOWN，不是 TRUE，所以不能通过 WHERE 或命中 CASE WHEN。

## 4、AND OR 优先级陷阱

SQL：看到 OR 就加括号，不要赌优先级。
DAX：看到 OR 就进 FILTER()，不要往逗号里塞。

### SQL中：
核心陷阱：OR "吃掉"了后面的 AND 条件
SQL 里 AND 优先级高于 OR（就像乘法优先于加法）。
错误示范：

```sql
-- ❌ 就像 DAX 里没用 && 把条件放一起，而是拆成逗号（但 SQL 里拆成 AND 会受优先级影响）
WHERE register_date <= '2026-05-23'
   OR __IsMemberFilter <> 1    -- 这行根本不能这么写，变量不能放 WHERE 里，但假设能放
   AND register_date IS NOT NULL
-- 实际解析：date <= '2026-05-23' OR (__IsMemberFilter <> 1 AND date IS NOT NULL)
-- 完全不是你想要的逻辑！
```
SQL 最佳实践:凡是 WHERE / ON 里出现 OR，立刻加括号，不要依赖优先级记忆。
```sql
-- ✅ 正确：把 OR 锁死在一个字段/一组条件的范围内
WHERE is_member = 0
  AND (register_date <= '2026-05-23' OR register_date IS NULL)
```
### Dax中：

#### 陷阱 1：CALCULATE 里逗号不能直接表达 OR

CALCULATE 的逗号 = AND，OR 必须进 FILTER()。

```dax
-- ✅ 写法 ：用 FILTER 包裹 OR 逻辑
CALCULATE(
    [Measure],
    'Table'[A] = 1,
    FILTER(ALL('Table'[B], 'Table'[C]), 'Table'[B] = 2 || 'Table'[C] = 3)
)
```

#### 陷阱 2：&& 和 || 混用时的优先级（和 SQL 一样）

&& 和 || 混用时,&&的优先级更高。
IF( A || (B && C), 1, 0 )
如果你本意是 (A || B) && C，结果就错了。

#### 陷阱 3：上下文转换 + 逗号 vs && 的行为差异

```dax
-- 写法 A：逗号分隔（两个独立筛选器）
CALCULATE( [Measure], 'T'[A] = 1, 'T'[B] = 2 )
...
powerbi连接bytehouse账号
用户：powerbi_rl_e2e_r
密码：vbUT3m-PyNDaGXq3
2104578120::powerbi_rl_e2e_r::bigdata

## ODBC信息

Data Source Name：bytehouse_rl
Description：ByteHouse Connection via MySQL ODBC
TCP/IP Server：bytehouse-hs-prod.cloud.bz
Port：33062
User：2104578120::powerbi_rl_e2e_r::bigdata
Password：vbUT3m-PyNDaGXq3

## 本地电脑配置截图

![1789634254108](image/知识点/1789634254108.png)
![1789634277301](image/知识点/1789634277301.png)
![1789634286081](image/知识点/1789634286081.png)
![1789634292503](image/知识点/1789634292503.png)
![1789634299079](image/知识点/1789634299079.png)
![1789634304868](image/知识点/1789634304868.png)
![1789634310330](image/知识点/1789634310330.png)
...
行-->颜色1： #BE9F71
数据标签1颜色： #BE9F71
数据标签-->背景1：#BE9F71、透明度60%

行-->颜色2： #CCCCCC
数据标签2颜色： #605E5C
数据标签-->背景2：#FFFFFF、透明度30%
柱子：
数据标签-->选项-->位置(列)：中心内
值-->颜色： #FFFFFF
