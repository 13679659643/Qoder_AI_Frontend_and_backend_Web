...
### 15. 事实表无关联：父行 = 子行之和
说明：把"四个维度全部等值匹配"改成"按当前展开层级动态匹配"：某一层没展开（SELECTEDVALUE 返回 BLANK）就不加该层的等值条件，父行自然等于子行之和。
用 IF(__X = BLANK(), TRUE(), 列 = __X) 作为 CALCULATE 的筛选参数：条件为 TRUE() 时等于不筛选。
案例：
```dax
...
### 16. KEEPFILTERS + 条件开关：保留外部筛选，按需追加限制

来源：[LY_Last_Purchase_Time_Table.md](file:///d:/gutao/辜涛/Project/Qoder_AI_Frontend_and_backend_Web/RL%20E2E/RL%20E2E%20Customer%20Dashboard/VIC/3%20LY%20Last%20Purchase%20Time/LY_Last_Purchase_Time_Table.md)（6 个 Base Act / Base LY 度量值）

- **适用场景**：断开切片器控制是否启用额外筛选；启用时与该列已有筛选取交集，关闭时保留原筛选。
- **核心区别**：`CALCULATE` 中普通布尔筛选默认替换同一列已有的筛选；`KEEPFILTERS(条件)` 改为“已有筛选 ∩ 新条件”，不会放宽该列原有范围。
- **条件开关**：`__IsMemberFilter = 0 || register_date <= __PeriodMax`。选择 0 时表达式恒真，不追加注册日期限制；选择 1 时仅保留不晚于期末的注册日期。这里依靠逻辑真值控制筛选，不代表引擎一定采用短路执行。
- **业务映射**：切片器 `IsMember` 的 0/1 是模式开关，不直接等于事实表筛选值。本案例两档均筛 `is_member = 0`，Member VIC 才追加注册日期上限。

**本期示例（沿用原方案 LY VIC No. 基础值）**：

```dax
_LY VIC No. Base Act =
// ========================================
// 用途: end period 当月 LY VIC No.，按会员模式追加注册日期限制
// 依赖: a03_e2e_customer_data_m、Slicer_Time_Frame_Max、
//       IsMemberFilter、Slicer_Is_Employee_Selection
// ========================================
    ...
    RETURN
        CALCULATE(
            ...
            'a03_e2e_customer_data_m'[is_member] = 0,
            // TTL: 不追加日期限制；Member: 与已有注册日期筛选取交集
            KEEPFILTERS(
                __IsMemberFilter = 0
...
**开关与交集对照**（下表只描述注册日期条件，其他指标筛选照常生效）：

| 切片器结果 | OR 条件 | KEEPFILTERS 后的注册日期范围 |
|---|---|---|
| 0：TTL VIC（默认） | 恒为 TRUE | 保留已有范围，不新增上限 |
| 1：Member VIC | `register_date <= __PeriodMax` | 已有范围 ∩ 不晚于期末的日期，包含期末当天 |

假设已有 `register_date` 筛选为 **2026-09-01 ~ 2026-09-10**，追加条件为 **不晚于 2026-09-25**：

| 写法 | 对同列已有筛选的作用 | 最终注册日期范围 |
|---|---|---|
| 普通布尔条件 `register_date <= __PeriodMax` | 替换 | 不晚于 2026-09-25，可能纳入原范围之外的日期 |
| `KEEPFILTERS(register_date <= __PeriodMax)` | 取交集 | 仍为 2026-09-01 ~ 2026-09-10 |
| TTL 模式下 `KEEPFILTERS(TRUE 条件)` | 与全部可选日期取交集 | 仍为 2026-09-01 ~ 2026-09-10，不清除已有筛选 |

**复用边界**：

- **时间成对切换**：本期使用 `Last_Fiscal_Month_Min/Max`；LY 使用 `Last_Fiscal_Month_Min_LY/Max_LY`，注册日期上限同步改用 `__LYMax`。本来源方案无 LP；扩展到含 LP 的方案时，应使用对应 LP 财月区间和 `Last_Fiscal_Month_Max_LP`。
- **作用范围有限**：本例 `KEEPFILTERS` 只保护 `register_date` 条件，不会自动改变其他 `CALCULATE` 参数的覆盖行为；`is_member = 0` 和 `data_date` 区间仍按原方案显式设置。
- **默认值与字段类型**：`SELECTEDVALUE(..., 0)` 在没有唯一选值时回退 TTL VIC；日期切片器需提供有效单值，`register_date` 应为 Date。BLANK 日期可能通过 DAX 的 `<=` 比较，是否排除须由业务口径决定，不能把 `KEEPFILTERS` 当成非空校验。
- **只复用筛选模式**：保留各指标原有聚合与专属条件，不引入新的分子、分母定义；本来源方案的 Retention% 仍为 Retention No. / LY VIC No.。

### 17. 哨兵日期恒真模式：register_date 上限变量化（TTL 档谓词恒真）

来源：[VIC_KPIs_Table.md](file:///d:/gutao/辜涛/Project/Qoder_AI_Frontend_and_backend_Web/RL%20E2E/RL%20E2E%20Customer%20Dashboard/VIC/1%20VIC%20KPI/VIC_KPIs_Table.md)（Act/LY/LP 三个 Base Value 的 VAR 定义区，共 15 处 CALCULATE 谓词统一复用）

- **适用场景**：与第 16 点同一业务背景——切片器 `IsMember` 的 0/1 是模式开关而非筛选值（两档均筛 `is_member = 0`，Member VIC 档才追加注册日期上限，TTL VIC 档不设限）。
- **核心机制**：不做行级条件开关（KEEPFILTERS + OR），而是把日期上限抽象为变量 `__EndPeriodDate`：Member 档取真实期末日，TTL 档取哨兵日期 `DATE(9999,12,31)`——任何真实 `register_date` 都不晚于哨兵值，谓词 `register_date <= __EndPeriodDate` 恒真，等价于“不设限”。
- **效果**：事实表谓词在所有 CALCULATE 中统一写 `is_member = 0` + `register_date <= __EndPeriodDate` 两行，TTL/Member 双分支自然合一，无需按分支复制两份 CALCULATE，也无需依赖 KEEPFILTERS 的交集语义。

**代码示例（Act Base Value 的 VAR 定义区）**：
...
// ── 后续所有 CALCULATE 谓词统一写两行，无需按 TTL/Member 分支复制 ──
//     'a03_e2e_customer_data_m'[is_member] = 0,
...
| 切片器选择 | __IsMemberFilter | __EndPeriodDate 取值 | register_date 谓词效果 |
|---|---|---|---|
| 0：TTL VIC（默认） | 0 | DATE(9999,12,31)（哨兵） | 恒真，等价于不设限 |
| 1：Member VIC | 1 | 对应期 end period 末日 | 仅保留不晚于期末的注册日期（含期末当天） |

**与第 16 点（KEEPFILTERS + OR 条件开关）的选型对照**：

| 维度 | 第 16 点：KEEPFILTERS + OR | 第 17 点：哨兵日期 |
|---|---|---|
| 开关位置 | 行级（`__IsMemberFilter = 0 \|\| register_date <= __PeriodMax` 逐行求值） | 变量级（IF 提前定死上限值） |
| 与同列已有筛选的关系 | KEEPFILTERS 取交集，保留外部已有 register_date 筛选 | 普通布尔筛选为替换语义；TTL 档哨兵替换后仍全量放行，但会覆盖外部 register_date 筛选 |
| 谓词书写 | 一处 KEEPFILTERS 条件表达式 | 多处 CALCULATE 统一两行谓词，书写完全一致 |
| 适用场景 | 外部存在 register_date 筛选、需保留交集 | 度量值全权接管 register_date 谓词（多 CALCULATE 统一书写、多期别复用同一变量） |

**复用边界**：

- **期间字段切换**：Act 取 `Last_Fiscal_Month_Max`；LY 取 `Last_Fiscal_Month_Max_LY`；LP 取 `Last_Fiscal_Month_Max_LP`（各期注册截止与各期基础聚合区间同源，勿跨期混用）。
- **哨兵值前提**：哨兵日期必须晚于事实表中所有可能的 register_date（`DATE(9999,12,31)` 为安全上界）；若业务日期可能超出哨兵值，模式失效。
- **BLANK 语义**：BLANK register_date 可能通过 `<=` 比较（被视为最小值）被保留；是否排除须由业务口径决定，哨兵模式本身不做非空校验（与第 16 点一致）。
- **只复用筛选模式**：不改变各指标原有聚合与专属条件（如 is_xxx_vic、data_date 区间）；register_date 谓词仅由 `__EndPeriodDate` 变量承载。
