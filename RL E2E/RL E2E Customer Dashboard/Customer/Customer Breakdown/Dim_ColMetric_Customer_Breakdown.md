Dim_ColMetric_Customer_Breakdown =
// ========================================
// 表: Dim_ColMetric_Customer_Breakdown
// 类型: 维度表（Dim_ 前缀），断开维度
// 用途: 定义 Customer Dashboard - Customer Tab 的 Customer Breakdown 矩阵列维度（KPIGroup > ColName）
// 范围: 口径文档/Customer/Performance Indicator.md - 子模块二（6 个 KPI 分组，共 18 列指标）
// 数据底表: a03_e2e_customer_data_m
//
// 与 Dim_ColMetric_Customer_Performance_Indicator 的差异（Breakdown 版本）:
//   1. 金额类 Metric_Format 从 "currency" 改为 "currency_M_K_Int_0db"
//      （分级显示：< 1K 用整数千分位，1K~1M 用 K 单位 1 位小数，≥ 1M 用 M 单位 1 位小数）
//   2. ColType 中的 "Act" 替换为对应分组名称（DCom SLS / Customer No. / ACV / AUR / Freq. / UPT）
//      （Breakdown 模块列指标本身就是分组维度，无需再保留 "Act" 标签）
//   3. 其余指标（vs LY / vs LP、颜色规则、Metric_IsCurrencyAmount）保持不变
//
// 设计原则（遵循口径文档要求）:
...
//   4. Metric_IsCurrencyAmount（BOOLEAN）字段：
...
//   ColType                  列类型标识：本版本 Act 列替换为对应分组名（DCom SLS / Customer No. / ACV / AUR / Freq. / UPT），vs LY / vs LP 不变
//   Metric_Format            单一格式字段（严格对应口径文档数据类型）
//                            取值: currency_M_K_Int_0db / integer / delta_pct_0dp
...
DATATABLE(
    ...
    "ColType",                  STRING,     // 列类型标识：本版本 Act 列替换为分组名；vs LY / vs LP 不变
    ...
    "Metric_ColorDefault",      STRING,     // 默认颜色
    {
        ...
        // ════════════════════════════════════════════════════════════════
        { 1,  "DCom SLS",     "1-DCom SLS",          10, 10, "DCom SLS",     "currency_M_K_Int_0db", TRUE,  "fixed_black",  "#1A9018", "#D64550", "#E1C233", "#5F6165" },
        ...
        // 颜色规则: Act=固定黑；vs LY/vs LP=正负零三色
        // ════════════════════════════════════════════════════════════════
        { 4,  "Customer No.", "4-Customer No.",          20, 20,  "Customer No.", "integer",       FALSE, "fixed_black",  "#1A9018", "#D64550", "#E1C233", "#5F6165" },
