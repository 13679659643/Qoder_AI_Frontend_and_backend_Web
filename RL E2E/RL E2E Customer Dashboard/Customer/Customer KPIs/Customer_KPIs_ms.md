...
### 1.2 DCom 新客判定 = Step1 + Step2 EXCEPT（不可合并区间）
...
技术实现采用 Step1（本期有消费的新客候选）EXCEPT Step2（第一财月的老客排除集）的方式。两步区间不同、不可合并（参考：维度复用/新客 No. 模板详解.md）：

- Step1（本期有消费的新客候选）：`data_date ∈ [__PeriodMin, __PeriodMax]`（本期区间），`is_member = 0`，`SUM(net_pay_amt) > 0`
- Step2（第一财月的老客排除集）：`data_date ∈ [__StartPeriodMin, __StartPeriodMax]`（第一个财月区间），`is_member = 0`，`SUM(lp_12m_net_pay_amt) > 0`
- 结果 = `COUNTROWS(EXCEPT(Step1, Step2))`（本期消费者剔除第一财月已识别的老客）
...
- `net_pay_amt > 0` 来自 Step 1（本期有消费）
- `is_member = 0` 来自 Step 1（Customer KPI 固定非会员）
- `lp_12m_net_pay_amt > 0` 来自 Step 2（老客排除集，第一财月已有 12 个月历史消费）
- Step1/Step2 内部按 `user_id + shop_info_id` 聚合用于 `SUM(net_pay_amt)`/`SUM(lp_12m_net_pay_amt)` 判定；最终计数按 `user_id` 去重（`SUMMARIZE(..., [user_id])` 再 `COUNTROWS`），人数只算 user_id 粒度

DAX 实现用 `SUMMARIZECOLUMNS` 按 `user_id + shop_info_id` 聚合 Step1/Step2 做 SUM 判定，再各自 `SUMMARIZE(..., [user_id])` 去重到用户级，最后 `COUNTROWS(EXCEPT(Step1, Step2))` 得到新客数。
...
[Customer KPIs Act Base Value]          ← 本期基础值
                                        ← Metric_ID=1: DCom New Customer No.（EXCEPT Step1/Step2）
...
| --------- | ------------ | ----------------------------- | ---------------- | ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| 1 | Customer No. | 1-DCom New Customer No. | Act | 1. DCom New Customer No. | EXCEPT(Step1, Step2)：本期区间 SUM(net_pay_amt)>0 ∩ is_member=0 减去 第一财月 SUM(lp_12m_net_pay_amt)>0 ∩ is_member=0；Step1/Step2 内部按 user_id+shop_info_id 聚合做 SUM 判定，最终按 user_id 去重计数 | a03_e2e_customer_data_m |
...
| 6 | Customer% | 6-DCom New Customer% | Act | 2. DCom New Customer% | 分子: Metric_ID=1（EXCEPT 新客数）；分母: COUNTROWS(SUMMARIZE(Step1,[user_id])) 本期区间 SUM(net_pay_amt)>0 ∩ is_member=0，按 user_id 去重计数（与分子同粒度） | a03_e2e_customer_data_m |
...
// 聚合粒度: Step1/Step2 内部 user_id+shop_info_id（SUM 判定）；最终计数按 user_id 去重（COUNTROWS(SUMMARIZE(...,[user_id]))）
// 说明:
//   - Metric_ID=1: DCom New Customer No.
...
//     Step1/Step2 内部按 user_id+shop_info_id 聚合做 SUM 判定；最终各自 SUMMARIZE 去重到 user_id 再 EXCEPT，人数按 user_id 粒度
//   - Metric_ID=6: DCom New Customer% = DIVIDE(分子 Metric_ID=1, 分母 全部顾客)
//     分子: 同 Metric_ID=1（DCom New Customer No.）
//     分母: COUNTROWS(SUMMARIZE(Step1,[user_id])) 本期区间内 SUM(net_pay_amt)>0 ∩ is_member=0 的全部顾客（按 user_id 去重，与分子同粒度，不分新老客）
...
// ========================================
    ...
    // Step1/Step2 内部按 user_id + shop_info_id 聚合做 SUM 判定；最终各自 SUMMARIZE 去重到 user_id 再 EXCEPT/COUNTROWS，人数按 user_id 粒度
    // ═══════════════════════════════════════
    VAR __NewCust_Step1 =
        SELECTCOLUMNS(
            FILTER(
    ...
    VAR __NewCustomerCnt_Act = COUNTROWS(
        EXCEPT (
    ...
    // 本期区间内全部有消费的顾客（user_id粒度，与新客分子保持一致）
    // = COUNTROWS(SUMMARIZE(__NewCust_Step1, [user_id]))（按 user_id 去重再计数）
    // ═══════════════════════════════════════
    VAR __AllCustomerCnt_Act = COUNTROWS ( SUMMARIZE ( __NewCust_Step1, [user_id] ) )   -- ✅ 按 user_id 去重再数
...
// 说明:
//   - Metric_ID=1: DCom New Customer No. LY（Step1+Step2 EXCEPT，时间范围改为 LY）
...
// ========================================
    ...
    VAR __NewCustomerCnt_LY = COUNTROWS(
        EXCEPT (
        SUMMARIZE ( __NewCust_Step1_LY, [user_id] ), -- 去重到用户级
        SUMMARIZE ( __OldCust_Step2_LY, [user_id] )
    ))

    // ═══════════════════════════════════════
    // 分母 LY — DCom New Customer% 分母（LY 区间内全部有消费的顾客，与分子同粒度）
    // = COUNTROWS(SUMMARIZE(__NewCust_Step1_LY, [user_id]))（按 user_id 去重再计数）
    // ═══════════════════════════════════════
    VAR __AllCustomerCnt_LY = COUNTROWS ( SUMMARIZE ( __NewCust_Step1_LY, [user_id] ) )   -- ✅ 按 user_id 去重再数
...
// ========================================
    ...
    // Step1+Step2 不能合并区间计算（参考：维度复用/新客 No. 模板详解.md）：
    //   Step1（上期有消费的新客候选）：data_date ∈ [__PeriodMin_LP, __PeriodMax_LP]，is_member = 0，SUM(net_pay_amt) > 0
    //   Step2（上期第一财月的老客排除集）：data_date ∈ [__StartPeriodMin_LP, __StartPeriodMax_LP]，is_member = 0，SUM(lp_12m_net_pay_amt) > 0
    //   结果 = COUNTROWS(EXCEPT(Step1, Step2))
    // Step1/Step2 内部按 user_id + shop_info_id 聚合做 SUM 判定；最终各自 SUMMARIZE 去重到 user_id 再 EXCEPT/COUNTROWS，人数按 user_id 粒度
    // ═══════════════════════════════════════
    ...
    VAR __NewCustomerCnt_LP = COUNTROWS(
        EXCEPT (
        SUMMARIZE ( __NewCust_Step1_LP, [user_id] ), -- 去重到用户级
    ...
    // ═══════════════════════════════════════
    // 分母 LP — DCom New Customer% 分母（LP 区间内全部有消费的顾客，与分子同粒度）
    // = COUNTROWS(SUMMARIZE(__NewCust_Step1_LP, [user_id]))（按 user_id 去重再计数）
    // ═══════════════════════════════════════
    VAR __AllCustomerCnt_LP = COUNTROWS ( SUMMARIZE ( __NewCust_Step1_LP, [user_id] ) )   -- ✅ 按 user_id 去重再数
...
| ---- | ----------------------------------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1    | Customer KPIs Act Base Value        | Base Metrics   | 本期基础值（EXCEPT Step1/Step2，COUNTROWS(SUMMARIZE(.,[user_id])) 按 user_id 去重计数）；Metric_ID=6 返回 DCom New Customer% 比率                                                                             |
...
1. **实际值时间口径**：实际值取数按**所选时间范围区间聚合**（`data_date ∈ [Slicer_Time_Frame_Min[TimeFrame_Min], Slicer_Time_Frame_Max[TimeFrame_Max]]`）。分母（DCom New Customer% 的全部顾客 = COUNTROWS(SUMMARIZE(Step1,[user_id]))）按本期区间聚合；分子（DCom New Customer No. = COUNTROWS(EXCEPT(SUMMARIZE(Step1,[user_id]), SUMMARIZE(Step2,[user_id])))）的 Step1 按本期区间聚合、Step2 按第一财月（start_period）聚合，两步区间不同、不可合并；人数均按 user_id 去重计数。
2. **DCom 新客判定 Step1 + Step2 EXCEPT 实现**（参考：维度复用/新客 No. 模板详解.md）：
   - Step1（本期有消费的新客候选）：`data_date ∈ [__PeriodMin, __PeriodMax]`（本期区间），`is_member = 0`，`SUM(net_pay_amt) > 0`
   - Step2（第一财月的老客排除集）：`data_date ∈ [__StartPeriodMin, __StartPeriodMax]`（第一个财月区间），`is_member = 0`，`SUM(lp_12m_net_pay_amt) > 0`
   - 结果 = `COUNTROWS(EXCEPT(SUMMARIZE(Step1,[user_id]), SUMMARIZE(Step2,[user_id])))`（本期消费者剔除第一财月已识别的老客，按 user_id 去重计数）
   - 两步区间不同、不可合并：Step1 用本期区间，Step2 用第一财月区间
   - Step1/Step2 内部按 `user_id + shop_info_id` 聚合做 SUM 判定；最终各自 `SUMMARIZE(..., [user_id])` 去重到 user_id 再 EXCEPT/COUNTROWS（人数按 user_id 粒度，非 user_id+shop_info_id）；platform/shop_info_id 由模型 1:N 关系自动筛选
