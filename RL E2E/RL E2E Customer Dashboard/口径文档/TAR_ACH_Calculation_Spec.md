...
> **data_date = 所选时间范围** | 涉及到a03_e2e_customer_fcst_data_m表计算，data_date = 所选时间范围，所选时间范围的计算,data_date ∈ [__TimeMin, __TimeMax]（全局时间范围），Slicer_Time_Frame_Min和Slicer_Time_Frame_Max维度表已经给出了具体的TimeFrame_Min和TimeFrame_Max值
...
| **聚合粒度** | 数字卡片：所选时间范围 `dt`（end period），所选时间范围的最后一个财月，只关注Max；表格：按对应维度聚合 |
...
| **目标值表说明** | 目标值均取自 `a03_e2e_customer_fcst_data_m`，日期字段为 `data_date`，按 `dt = 所选时间范围`，`group by platform, shop_info_id` 聚合 |
| **data_date = 所选时间范围** | 涉及到a03_e2e_customer_fcst_data_m表计算，data_date = 所选时间范围，所选时间范围的计算,data_date ∈ [__TimeMin, __TimeMax]（全局时间范围），Slicer_Time_Frame_Min和Slicer_Time_Frame_Max维度表已经给出了具体的TimeFrame_Min和TimeFrame_Max值 |
...
| **计算公式** | 实际值 / 目标值 |
| **实际值** | VIC No. — count(distinct user_id) where `is_vic = 1` |
...
| **计算公式** | 实际值 / 年目标值 |
| **实际值** | VIC Retention% — count(distinct user_id) where `is_retention_vic = 1` / count(distinct user_id) where `is_vic = 1`（上一周期） |
| **目标值取数逻辑** | `data_date = 所选时间范围`，data_date ∈ [TimeFrame_Min, TimeFrame_Max]（全局时间范围），目标值 = DISTINCT `year_vic_retention_percent` |
| **目标值底表** | `a03_e2e_customer_fcst_data_m`，日期字段 `data_date` |
| **数据底表（实际值）** | `a03_e2e_customer_data_m` |
...
**TAR ACH% 计算规则矩阵（VIC Retention%）**

| TimeFrame | 选择范围 | 分子 | 分母 | 说明 |
|---|---|---|---|---|
| Month | 选择单个财月 | VIC Retention%（month actual） | DISTINCT `year_vic_retention_percent` | 单月留存率实际 / 年度留存率目标 |
| Month | 选择多个财月 | — | — | **留空**，多个月不计算TAR ACH% |
| Month | 选择多个财月且跨财年 | — | — | **留空** |
| Quarter | 任意选择范围 | — | — | **留空**，Quarter下不计算TAR ACH% |
| Year | 选择单个财年 | VIC Retention%（year actual） | DISTINCT `year_vic_retention_percent` | 年度留存率实际 / 年度留存率目标 |
...
| **计算公式** | 实际值总和 / 年目标值 |
| **实际值** | T4-5 Upgrade No. — count(distinct user_id) where `is_upgrade_vic = 1` |
...
**TAR ACH% 计算规则矩阵（T4-5 Upgrade No.）**
...
| **数据底表（实际值）** | `a03_e2e_customer_data_m`、`t05_customer_order_data_d` |
| **数据底表（目标值）** | `a03_e2e_customer_fcst_data_m`，日期字段 `data_date` |
| **目标值聚合方式** | `dt = 所选时间范围`，`group by platform, shop_info_id` |
| **筛选逻辑** | 模块全局影响：除了特殊说明之外的指标不需要判断is_member和is_employee，其余都默认需要判断is_member和is_employee来确定筛选事实表的值 |
| **货币转换规则** | 数据源默认为 RMB，转化为美元需要除以固定值 7 |
