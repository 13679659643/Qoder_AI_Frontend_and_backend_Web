...
> **分组维度**: 按 New/Existing/All 客户类型，按 `platform, shop_info_id` 维度聚合。

### 1. Net 维度

#### 1.1 SLS Breakdown（Net） — 净销售额

| 项目 | 内容 |
|---|---|
| **指标名称** | SLS Breakdown（Net） |
| **指标名称中文** | 净销售额 |
| **业务定义** | 净销售额 |
| **计算公式** | `sum(net_pay_amt)`， All 分支的情况 |
| **统计字段** | `net_pay_amt` |
...
| **附属指标数据类型** | percent_0dp → 百分比整数，不含正号 |
| **附属指标数据格式** | `#,##0%` |
...
| **指标名称中文** | 新客净销售额 |
| **业务定义** | 新客净销售额 |
| **计算公式** | **New**：Step1（本期有消费的新客候选）：`data_date ∈ slicer 区间 AND is_member = 0 AND SUM(net_pay_amt) > 0`；Step2（第一财月的老客排除集）：`data_date ∈ start_period AND is_member = 0 AND SUM(lp_12m_net_pay_amt) > 0`；New user_id 集 = `EXCEPT(SUMMARIZE(Step1,[user_id]), SUMMARIZE(Step2,[user_id]))`；再在 slicer 区间内对该集合 `SUM(net_pay_amt) WHERE is_member = 0`。两步区间不同、不可合并。 |
| **数据底表** | `a03_e2e_customer_data_m` |
| **筛选条件** | Step1：`SUM(net_pay_amt) > 0`，`is_member = 0`（slicer 区间）；Step2：`SUM(lp_12m_net_pay_amt) > 0`，`is_member = 0`（start_period）；New = Step1 EXCEPT Step2 |
| **聚合粒度** | `data_date = 所选时间范围`，`platform, shop_info_id` |
| **数据类型** | currency_M_K_Int_0db → 值 < 1,000        → 货币符号 + 千分位整数：¥999;1,000 ≤ 值 < 1M   → 货币符号 + K 单位（1位小数）：¥1.5K;值 ≥ 1,000,000    → 货币符号 + M 单位（1位小数）：¥1.5M; |
| **数据格式** | dax```IF( __Value < 1000,__CurrencySymbol & FORMAT(__Value, "#,##0"),IF(__Value < 1000000,__CurrencySymbol & FORMAT(__Value / 1000, "#,##0.0") & "K",__CurrencySymbol & FORMAT(__Value / 1000000, "#,##0.0") & "M")), // ¥999\¥1.5K\¥1.5M; ```|
| **附属指标** | vs LY：今年 / 去年 - 1；vs LP：当期 / 上期 - 1 ；New Customer SLS Share — 新客销售额占比：分子原指标，分母 ：New 分支 + Existing 分支；|
| **附属指标数据类型** | percent_0dp → 百分比整数，不含正号 |
| **附属指标数据格式** | `#,##0%` |
...
| **指标名称中文** | 老客净销售额 |
| **业务定义** | 老客净销售额 |
| **计算公式** | **Existing**：Step1 同 New，但与 Step2 取 `INTERSECT`（本期消费者中第一财月已识别的老客，`lp_12m_net_pay_amt > 0`）；再在 slicer 区间内对该集合 `SUM(net_pay_amt) WHERE is_member = 0`。两步区间不同、不可合并。 |
| **数据底表** | `a03_e2e_customer_data_m` |
| **筛选条件** | Step1：`SUM(net_pay_amt) > 0`，`is_member = 0`（slicer 区间）；Step2：`SUM(lp_12m_net_pay_amt) > 0`，`is_member = 0`（start_period）；Existing = Step1 INTERSECT Step2 |
| **聚合粒度** | `data_date = 所选时间范围`，`platform, shop_info_id` |
| **数据类型** | currency_M_K_Int_0db → 值 < 1,000        → 货币符号 + 千分位整数：¥999;1,000 ≤ 值 < 1M   → 货币符号 + K 单位（1位小数）：¥1.5K;值 ≥ 1,000,000    → 货币符号 + M 单位（1位小数）：¥1.5M; |
| **数据格式** | dax```IF( __Value < 1000,__CurrencySymbol & FORMAT(__Value, "#,##0"),IF(__Value < 1000000,__CurrencySymbol & FORMAT(__Value / 1000, "#,##0.0") & "K",__CurrencySymbol & FORMAT(__Value / 1000000, "#,##0.0") & "M")), // ¥999\¥1.5K\¥1.5M; ```|
| **附属指标** | Existing Customer SLS Share — 老客销售额占比：分子原指标，分母 ：New 分支 + Existing 分支； |
| **附属指标数据类型** | percent_0dp → 百分比整数，不含正号 |
| **附属指标数据格式** | `#,##0%` |


#### 1.4 Customer No. Breakdown（Net） — 净购买买家人数

| 项目 | 内容 |
|---|---|
| **指标名称** | Customer No. Breakdown（Net） |
| **指标名称中文** | 净购买买家人数 |
| **业务定义** | 净购买买家人数 |
...
| **统计字段** | `user_id` |
| **数据底表** | `a03_e2e_customer_data_m` |
| **筛选条件** | **New**：Step1（本期有消费的新客候选）：`data_date ∈ slicer 区间 AND is_member = 0 AND SUM(net_pay_amt) > 0`；Step2（第一财月的老客排除集）：`data_date ∈ start_period AND is_member = 0 AND SUM(lp_12m_net_pay_amt) > 0`；New user_id 集 = `EXCEPT(SUMMARIZE(Step1,[user_id]), SUMMARIZE(Step2,[user_id]))`，`COUNTROWS` 该集合（按 user_id 粒度计数）。两步区间不同、不可合并 |
| **聚合粒度** | `data_date = 所选时间范围`，`platform, shop_info_id` |
| **数据类型** | integer → 整数，千分位整数 |
| **数据格式** | `#,##0` |
| **附属指标** | New Customer No. Share：分子原指标，分母：New 分支 + Existing 分支；vs LY：今年 / 去年 - 1；vs LP：当期 / 上期 - 1 |
| **附属指标数据类型** | percent_0dp → 百分比整数，不含正号 |
| **附属指标数据格式** | `#,##0%` |
...
| **统计字段** | `user_id` |
| **数据底表** | `a03_e2e_customer_data_m` |
| **筛选条件** | **Existing**：Step1 同 New，但与 Step2 取 `INTERSECT`（本期消费者中第一财月已识别的老客，`lp_12m_net_pay_amt > 0`），`COUNTROWS(INTERSECT(SUMMARIZE(Step1,[user_id]), SUMMARIZE(Step2,[user_id])))`（按 user_id 粒度计数）。两步区间不同、不可合并 |
| **聚合粒度** | `data_date = 所选时间范围`，`platform, shop_info_id` |
| **数据类型** | integer → 整数，千分位整数 |
| **数据格式** | `#,##0` |
| **附属指标** | Existing Customer No. Share：分子原指标，分母 ：New 分支 + Existing 分支； |
| **附属指标数据类型** | percent_0dp → 百分比整数，不含正号 |
| **附属指标数据格式** | `#,##0%` |

### 2. Demand 维度

Net和Demand指标个数、逻辑都一致，区别仅在于字段，比如：Net部分字段为net_pay_amt、net_pay_qty、net_pay_order_cnt，Demand部分字段为pay_amt、pay_qty、pay_order_cnt。
