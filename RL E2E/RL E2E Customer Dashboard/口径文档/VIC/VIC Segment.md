...
> **模块全局影响说明**: 会员与员工筛选共同生效。TTL VIC（IsMember=0）：事实表 `is_member = 0`，不追加注册日期限制；Member VIC（IsMember=1）：事实表 `is_member = 0 AND register_date <= end_period_date`；员工筛选保持原规则。
> **is_member使用**: `VAR __IsMemberFilter = SELECTEDVALUE(IsMemberFilter[IsMember], 0)`，没有唯一选值时默认 TTL VIC。`IsMemberFilter` 与事实表保持断开关系；两档均筛选 `a03_e2e_customer_data_m[is_member] = 0`，仅 Member VIC 通过 `KEEPFILTERS` 追加 `register_date <= end_period_date`，与已有注册日期筛选取交集。
...
|---|---|
| **数据底表** | `a03_e2e_customer_data_m`、`t05_customer_order_data_d` |
| **筛选逻辑** | VIC Segment 所有计算指标及其分子、分母均执行全局会员规则：两档 is_member=0，Member VIC 通过 KEEPFILTERS 追加 register_date <= 对应本期/LY 最后财月末；Step 1 / Step 2 均生效，员工筛选保持原规则 |
...
| **模块全局影响说明** | TTL VIC（IsMember=0）：事实表 `is_member = 0`，不追加注册日期限制；Member VIC（IsMember=1）：事实表 `is_member = 0 AND register_date <= end_period_date`；员工筛选保持原规则 |
| **is_member使用** | `VAR __IsMemberFilter = SELECTEDVALUE(IsMemberFilter[IsMember], 0)`，默认 TTL VIC；断开维度仅控制是否追加注册截止条件，不将切片器值直接作为事实表 is_member 的值。事实表固定 `is_member = 0`；`KEEPFILTERS` 在 Member VIC 下追加 `register_date <= end_period_date`，TTL VIC 保留已有注册日期筛选、不新增上限 |
| **is_employee使用** | VAR __IsEmployeeFilter = SELECTEDVALUE(Slicer_Is_Employee_Selection[IsEmployee_Code], 1)，如果没有筛选，则默认Yes。这样过滤事实表a03_e2e_customer_data_m[is_employee] = __IsEmployeeFilter |
| **is_member和is_employee维度表路径** | is_member： D:\gutao\辜涛\Project\Qoder_AI_Frontend_and_backend_Web\RL E2E\RL E2E Customer Dashboard\维度复用\IsMemberFilter；is_employee： D:\gutao\辜涛\Project\Qoder_AI_Frontend_and_backend_Web\RL E2E\RL E2E Customer Dashboard\维度复用\Slicer_Is_Employee_Selection |
| **end period说明** | 所选时间范围的最后一个财月，直接读取 Slicer_Time_Frame_Max 的 Last_Fiscal_Month_Min/Max 及对应 LY/LP 字段，不自行推算自然月末。会员注册截止日 end_period_date 为对应最后财月的最后一天：本期 Last_Fiscal_Month_Max，LY Last_Fiscal_Month_Max_LY，LP Last_Fiscal_Month_Max_LP；本模块当前仅有 Act/LY，不新增 LP 指标 |
...
| **指标名称中文** | 买家人数占比 |
| **业务定义** | 该分层下买家人数/总买家数量 |
| **计算公式** | 分子：count(distinct user_id) where customer_tier = T1/T2/T3/T4/T5；分母：count(distinct user_id) where customer_tier in (T1/T2/T3/T4/T5) |
| **分子** | `user_id` |
| **分母** | `user_id`（全部 customer_tier in T1/T2/T3/T4/T5，与分子完全对称、无 net_pay_amt > 0 附加条件，移除行上下文 customer_tier 影响但保留外部切片器影响，ALLSELECTED；Total = T1+T2+T3+T4+T5 加总） |
| **数据底表** | `a03_e2e_customer_data_m` |
| **筛选条件** | 执行全局会员筛选规则（两档 is_member=0，Member VIC 追加注册截止条件）及原 is_employee 筛选 |
| **聚合粒度** | `dt = 所选时间范围 end period`，`platform, shop_info_id`分组维度由表字段自动传递，DAX 无需显式处理 |
| **数据类型** | percent_1dp → 百分比，保留一位小数，不含正号 |
