...
//
// Metric_Format 取值与口径文档数据类型对应关系（以参考列维度表 Dim_ColMetric_Fulfillment_PB_Merchandise.md 为准）:
//   integer         → `#,##0`                              整数千分位
//   percent_1dp     → `#,##0.0%`                           百分比一位小数，不含正号（TAR ACH% / VIC Retention% / Share / Retention VIC No. vs LY）
//   delta_pct_1dp   → `IF(__Value>0,"+","") & FORMAT(__Value,"#,##0.0%")`  百分比变化，含正号（数量类 vs LY / vs LP）
//   delta_pts       → `+#,##0pts;-#,##0pts;0pts`           基点（pts），含正负号，值×100 转 pts 在 Cell Display 中实现
//
// 注: 不存在 percent_1dp_signed / percent_1dp_nosign 两个格式。
//     所有"不含正号的百分比"统一为 percent_1dp；
//     所有"含正号的百分比变化"统一为 delta_pct_1dp。
...
DATATABLE(
    ...
    "Metric_ColorDefault",    STRING,     // 默认颜色
    {
        ...
        // 口径: 分子 is_retention_vic=1（end period 当月）/ 分母 is_vic=1（Rolling 12 个财月区间 DISTINCT 汇总）
        // 颜色规则: Act=固定黑；vs LY/vs LP=正负零三色；TAR ACH%=正负零三色
        // ════════════════════════════════════════════════════════════════
        ...
        //       Share 分子 is_upgrade_vic=1，分母 is_vic=1（end period 当月）
        ...
        //       Share 分子 is_retention_vic=1，分母 is_vic=1（end period 当月）
        // 注意 4.1 vs LY 数据类型为 percent_1dp（严格遵循口径文档，不含正号）
        ...
        //       Share 分子 is_direct_vic=1，分母 is_vic=1（end period 当月）
        // 颜色规则: Act/Share=默认色；vs LY/vs LP/Share vs LY/Share vs LP=正负零三色
        // ════════════════════════════════════════════════════════════════
