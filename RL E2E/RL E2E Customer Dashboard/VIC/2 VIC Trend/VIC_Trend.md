...
> **日期表结构假设**：Slicer_Time_Frame_VIC_Trend 的结构与原 Slicer_Time_Frame_Max 一致（包含 TimeFrame 系列 + Last_Fiscal_Month 系列），每个 X 轴时间点都有完整的 Last_Fiscal_Month 系列字段，用于 end period 逻辑和分母目标月推导（按 TimeFrame_Value 查任意月份的 TimeFrame_Min/TimeFrame_Max）。Slicer_Time_Frame_Max_VIC_Trend / Slicer_Time_Frame_Min_VIC_Trend 的结构与原 Slicer_Time_Frame_Max / Slicer_Time_Frame_Min 一致。
