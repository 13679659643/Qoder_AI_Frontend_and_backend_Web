...
| **TAR ACH%**                                                    | 占位值 1                          | `percent_1dp_signed`（含正负号）               |
| **Retention VIC No. vs LY**（数量类特殊）                       | 今年 / 去年 - 1                   | `percent_1dp_nosign`（不含正号，口径文档明确） |

### 1.4 关键特殊逻辑四：Rolling 12 个财月分母

VIC Retention% 的分母定义为：

> 所选时间范围 end period 往前 Rolling 12 个财月 `count(distinct user_id) where is_vic = 1`
> Rolling 12 个财月 = 当前月 + 往前 11 个月，共 12 个月

由于 `a03_e2e_customer_data_m` 是月度聚合表（每月一行 per user），分母等价于：在 end period 这个月（即 `Last_Fiscal_Month_Min` ~ `Last_Fiscal_Month_Max`）下 `is_vic=1` 的 distinct user_id 数。理由：表中 `is_vic=1` 字段本身已隐含 Rolling 12 个月判定逻辑（口径文档说明"在指定日期范围往前 Rolling 12 个财月，net sales >= 20k 的买家数量"对应 `dt = end period` 时的 `is_vic=1`）。

因此分母直接使用 end period 当月的 `is_vic=1` 人数，无需显式聚合 12 个月数据。
...
| 维度表                       | 类型     | 连接方式                                                                                            |
| ---------------------------- | -------- | --------------------------------------------------------------------------------------------------- |
| Slicer_Time_Frame_Max        | 断开维度 | SELECTEDVALUE 读取`Last_Fiscal_Month_Min/Max/Min_LY/Max_LY/Min_LP/Max_LP`                         |
...
[VIC KPIs LY Base Value]   ← 去年同期基础值（财历映射）
                            ← 使用 Last_Fiscal_Month_Min_LY ~ Last_Fiscal_Month_Max_LY
[VIC KPIs LP Base Value]   ← 上期基础值（财历映射）
                            ← 使用 Last_Fiscal_Month_Min_LP ~ Last_Fiscal_Month_Max_LP
...
| 18        | Retention VIC No. vs LY       | 数量类 vs LY（特殊格式） | 今年 / 去年 - 1     | percent_1dp_nosign |
...
| --------- | ----------------------- | ----------------------------------------------- | ------------------------------------- | ------------------ |
| 14        | T4-5 Upgrade No. Share  | `is_upgrade_vic=1` 的 DISTINCTCOUNT user_id   | `is_vic=1` 的 DISTINCTCOUNT user_id | percent_1dp_nosign |
| 20        | Retention VIC No. Share | `is_retention_vic=1` 的 DISTINCTCOUNT user_id | `is_vic=1` 的 DISTINCTCOUNT user_id | percent_1dp_nosign |
| 26        | Direct VIC No. Share    | `is_direct_vic=1` 的 DISTINCTCOUNT user_id    | `is_vic=1` 的 DISTINCTCOUNT user_id | percent_1dp_nosign |
...
| `percent_1dp_signed` | `IF(__Value>0,"+","") & FORMAT(__Value,"#,##0.0%")`；零值用 `0.0%`                 | +14.5% / -3.2% / 0.0%   | TAR ACH% 类指标（4, 5, 9, 13）                                                            |
| `percent_1dp_nosign` | `#,##0.0%`                                                                           | 14.5%                   | VIC Retention% / Share / Retention VIC No. vs LY                                          |
...
| 6         | VIC Retention%    | 6-VIC Retention%                 | Act              | 2. VIC Retention%                 | 分子: is_retention_vic=1；分母: is_vic=1 | a03_e2e_customer_data_m |
...
| 14        | T4-5 Upgrade No.  | 14-T4-5 Upgrade No. Share        | Share            | 3.4 T4-5 Upgrade No. Share        | 分子: is_upgrade_vic=1；分母: is_vic=1   | a03_e2e_customer_data_m |
...
| 20        | Retention VIC No. | 20-Retention VIC No. Share       | Share            | 4.3 Retention VIC No. Share       | 分子: is_retention_vic=1；分母: is_vic=1 | a03_e2e_customer_data_m |
...
| 26        | Direct VIC No.    | 26-Direct VIC No. Share          | Share            | 5.3 Direct VIC No. Share          | 分子: is_direct_vic=1；分母: is_vic=1    | a03_e2e_customer_data_m |
...
// 说明: VIC Retention% 和 Share 类指标在此处返回分子，分母在 Base Value 中单独计算
// ========================================
...
//
// Metric_ID 路由规则:
//   Act 基础指标 ID（基础值 = Act Base Value）：1, 6, 10, 14, 17, 20, 23, 26
//   数量类 vs LY 派生：2（VIC No. vs LY）, 11（T4-5 vs LY）, 24（Direct vs LY）
//   数量类 vs LP 派生：3（VIC No. vs LP）, 12（T4-5 vs LP）, 19（Retention vs LP）, 25（Direct vs LP）
//   数量类 vs LY 特殊格式：18（Retention VIC No. vs LY，使用 percent_1dp_nosign 但计算方式仍是 今年/去年-1）
//   比率类 vs LY 派生：7（Retention% vs LY）, 15（T4-5 Share vs LY）, 21（Retention Share vs LY）, 27（Direct Share vs LY）
//   比率类 vs LP 派生：8（Retention% vs LP）, 16（T4-5 Share vs LP）, 22（Retention Share vs LP）, 28（Direct Share vs LP）
//   TAR ACH% 占位：4, 5, 9, 13 → 返回 1（待逻辑确认后填充）
//
// vs LY 派生规则:
//   - 数量类：今年 / 去年 - 1
//   - 比率类（Retention% / Share）：今年 - 去年（差值，×100 转 pts 在 Cell Display 中实现）
//
// vs LP 派生规则:
//   - 数量类：当期 / 上期 - 1
//   - 比率类（Retention% / Share）：当期 - 上期（差值，×100 转 pts 在 Cell Display 中实现）
//
// Share 类计算:
//   - 分子 = 对应 is_xxx_vic=1 的 DISTINCTCOUNT(user_id)（即 Act Base Value 的同字段聚合）
//   - 分母 = is_vic=1 的 DISTINCTCOUNT(user_id)
//   - 直接通过 Act Base Value 在分子 Metric_ID（14/20/26）和分母 Metric_ID（1/6/17/23 的 is_vic 等价）上分别取值
//   - Share vs LY / Share vs LP 为 Share 的派生（比率类差值，×100 转 pts）
//
// REMOVEFILTERS 机制（参考 PB_Merchandise_Fulfillment_detail_ms.md）:
//   矩阵行标题会保留断开维度的所有列筛选器，
//   仅覆盖 Metric_ID 会导致筛选条件冲突，从而返回 BLANK。
//   因此派生行需先 REMOVEFILTERS 清除断开维度的所有筛选，再应用目标 Metric_ID。
// ========================================
    VAR __MetricID = SELECTEDVALUE('Dim_ColMetric_VIC_KPIs'[Metric_ID])

    // ═══════════════════════════════════════
    // 派生指标：通过 REMOVEFILTERS + 目标 Metric_ID 取 Act/LY/LP 值
    // ═══════════════════════════════════════
    ...
    // ═══════════════════════════════════════
    // 比率类（Retention% / Share）Act / LY / LP 取值
    // 比率类指标 = 分子 / 分母，分母统一为 is_vic=1 的 DISTINCTCOUNT(user_id)
    // ═══════════════════════════════════════
    // VIC Retention% 分组（Metric_ID 6/7/8）: 分子 is_retention_vic=1 → Act Base 在 Metric_ID=6 取值
    //   分母 is_vic=1 → 等价于 VIC No. Act（Metric_ID=1）的聚合
    // T4-5 Upgrade No. Share 分组（Metric_ID 14/15/16）: 分子 is_upgrade_vic=1 → Act Base 在 Metric_ID=14 取值
    //   分母 is_vic=1 → 等价于 VIC No. Act（Metric_ID=1）
    // Retention VIC No. Share 分组（Metric_ID 20/21/22）: 分子 is_retention_vic=1 → Act Base 在 Metric_ID=20 取值
    //   分母 is_vic=1 → 等价于 VIC No. Act（Metric_ID=1）
    // Direct VIC No. Share 分组（Metric_ID 26/27/28）: 分子 is_direct_vic=1 → Act Base 在 Metric_ID=26 取值
    //   分母 is_vic=1 → 等价于 VIC No. Act（Metric_ID=1）

    // 比率类 Act 值（仅 Metric_ID 6/14/20/26 是 Act，直接通过 Act Base Value 取）
    // 比率类 vs LY 派生: Metric_ID 7/15/21/27
    // 比率类 vs LP 派生: Metric_ID 8/16/22/28
    VAR __IsRatioVsLY = __MetricID IN {7, 15, 21, 27}
    VAR __IsRatioVsLP = __MetricID IN {8, 16, 22, 28}
    VAR __IsRatioDerived = __IsRatioVsLY || __IsRatioVsLP

    // 分子 Metric_ID（取 Act/LY/LP 的基础值时使用）
    VAR __RatioNumeratorMetricID =
        SWITCH(__MetricID,
            6, 6,   // VIC Retention% Act
            7, 6,   // VIC Retention% vs LY → 分子 Act Metric_ID=6
            8, 6,   // VIC Retention% vs LP → 分子 Act Metric_ID=6
    ...
    // 比率类分子 Act 值
    VAR __RatioNumeratorAct =
        IF(
            __IsRatioDerived,
            CALCULATE(
                [VIC KPIs Act Base Value],
                REMOVEFILTERS('Dim_ColMetric_VIC_KPIs'),
                'Dim_ColMetric_VIC_KPIs'[Metric_ID] = __RatioNumeratorMetricID
            )
        )
    // 比率类分子 LY 值
    VAR __RatioNumeratorLY =
        IF(
            __IsRatioVsLY,
            CALCULATE(
                [VIC KPIs LY Base Value],
    ...
    // 分母（is_vic=1，所有比率类共享）：等价于 VIC No. Act/LY/LP
    VAR __VICCountAct =
        IF(
            __MetricID IN {6, 14, 20, 26} || __IsRatioDerived,
            CALCULATE(
                [VIC KPIs Act Base Value],
                REMOVEFILTERS('Dim_ColMetric_VIC_KPIs'),
    ...
    VAR __RatioAct =
        IF(
            __MetricID IN {6, 14, 20, 26},
            DIVIDE(__RatioNumeratorAct, __VICCountAct)
        )
    // 比率类 vs LY（差值，展示时 ×100 转 pts）
    VAR __RatioVsLYResult =
        IF(
            __IsRatioVsLY,
            DIVIDE(__RatioNumeratorAct, __VICCountAct) - DIVIDE(__RatioNumeratorLY, __VICCountLY)
        )
    // 比率类 vs LP（差值，展示时 ×100 转 pts）
    VAR __RatioVsLPResult =
        IF(
            __IsRatioVsLP,
            DIVIDE(__RatioNumeratorAct, __VICCountAct) - DIVIDE(__RatioNumeratorLP, __VICCountLP)
        )
    ...
    RETURN
        SWITCH(
            ...
            // ─── 比率类 vs LY 派生（今年 - 去年，差值 ×100 转 pts）───
            7,  __RatioVsLYResult,               // VIC Retention% vs LY
            15, __RatioVsLYResult,               // T4-5 Upgrade No. Share vs LY
            21, __RatioVsLYResult,               // Retention VIC No. Share vs LY
            27, __RatioVsLYResult,               // Direct VIC No. Share vs LY
            // ─── 比率类 vs LP 派生（当期 - 上期，差值 ×100 转 pts）───
            8,  __RatioVsLPResult,               // VIC Retention% vs LP
            16, __RatioVsLPResult,               // T4-5 Upgrade No. Share vs LP
            22, __RatioVsLPResult,               // Retention VIC No. Share vs LP
            28, __RatioVsLPResult,               // Direct VIC No. Share vs LP
...
// 格式类型（严格遵循口径文档数据类型定义）:
//   integer              → 千分位整数：1,000
//   percent_1dp_signed   → 百分比一位小数含正负号：+14.5% / -3.2% / 0.0%
//                          （TAR ACH% 专用，零值用 0.0%）
//   percent_1dp_nosign   → 百分比一位小数不含正号：14.5%
//                          （VIC Retention% / Share / Retention VIC No. vs LY）
    ...
    RETURN
        IF(
            ...
            SWITCH(
                ...
                // ─── 百分比一位小数含正负号（TAR ACH%）──────────
                "percent_1dp_signed",
                    IF(__Value > 0, "+", "") & FORMAT(__Value, "#,##0.0%;-#,##0.0%;0.0%"),                              // +14.5% / -3.2% / 0.0%
                // ─── 百分比一位小数不含正号 ──────────────────────
                "percent_1dp_nosign",
...
//
// 颜色取值:
//   正值（>0）：Metric_ColorPositive（默认 #1A9018）
//   负值（<0）：Metric_ColorNegative（默认 #D64550）
//   零值（=0）：Metric_ColorZero（默认 #E1C233）
//   默认      ：Metric_ColorDefault（默认 #5F6165）
//   固定黑色  ：#252423
// ========================================
    VAR __Value = [VIC KPIs Cell Value]
...
| ---- | ------------------------------ | -------------- | ------------------------------------------------------------------------------------------------------------------- |
| 1    | VIC KPIs Act Base Value        | Base Metrics   | 本期基础值（end period 当月 DISTINCTCOUNT，按 Metric_ID 路由 is_vic/is_retention_vic/is_upgrade_vic/is_direct_vic） |
...
| 4    | VIC KPIs Base Value            | Base Metrics   | 总路由（含 vs LY / vs LP / TAR ACH% / Share 派生 + REMOVEFILTERS）                                                  |
...
-- VIC Retention%（分子 is_retention_vic=1，分母 is_vic=1）
SELECT
    COUNT(DISTINCT CASE WHEN is_retention_vic = 1 THEN user_id END) * 1.0
    / COUNT(DISTINCT CASE WHEN is_vic = 1 THEN user_id END) AS VIC_Retention_Pct_Actual
FROM a03_e2e_customer_data_m
WHERE is_member = 0
  AND is_employee = 1
  AND data_date BETWEEN '2026-09-01' AND '2026-09-30';
...
4. **Rolling 12 个财月分母等价处理**：VIC Retention% 的分母口径定义为"end period 往前 Rolling 12 个财月 count(distinct user_id) where is_vic=1"。由于 `a03_e2e_customer_data_m` 表中 `is_vic=1` 字段本身已隐含 Rolling 12 个月判定逻辑（在 `dt = end period` 时已计算），分母等价于 end period 当月 `is_vic=1` 的 DISTINCTCOUNT user_id。
5. **TAR ACH% 占位**：Metric_ID 4, 5, 9, 13 为占位指标，值固定为 1，待口径文档逻辑确认后再填充实际计算。
6. **Retention VIC No. vs LY 特殊格式**：Metric_ID 18（Retention VIC No. vs LY）的计算方式仍是 `今年 / 去年 - 1`（数量类），但数据格式为 `percent_1dp_nosign`（不含正号），严格遵循口径文档 4.1 的数据类型定义。
7. **Metric_ID 编码规则**：
...
8. **颜色规则三值标识**：通过 `Metric_ColorRule` 字段三值（`fixed_black` / `pos_neg_zero` / `fixed_default`）统一调度字体颜色，便于后续扩展。
...
9. **行维度处理**：无行维度表，直接拉取事实表字段（`platform` / `shop_info_id`），天然形成筛选与分组，DAX 度量值无需显式处理。模型自动传递筛选，支持 platform 粒度行展开看 shop_info_id 粒度明细数据。
10. **单一 Metric_Format 字段**：列指标维度表仅保留单个 `Metric_Format` 字段（不再区分 Act/LY/VsLY），因为每个指标对应一个格式。行格式严格遵循口径文档数据类型定义。
11. **扩展格式支持**：Cell Display 已扩展 `percent_0dp` / `percent_0dp_signed` / `percent_2dp` / `delta_pct_2dp` / `delta_pts_2dp` / `delta_bp` 等格式，便于后续快速调整。如需新增指标使用扩展格式，只需在 `Dim_ColMetric_VIC_KPIs` 的 `Metric_Format` 字段填入对应格式值即可。
12. **与 PB_Merchandise_Fulfillment_detail_ms.md 的关系**：本方案为 Customer Dashboard VIC Tab 的 KPI 矩阵 SWITCH 路由版本，与 PB Merchandise Fulfillment 版本共享相同的架构范式（断开列维度 + SWITCH 动态路由 + REMOVEFILTERS 修复上下文），差异在于：
