-- ClickHouse 日期范围生成器 - 周大福年累计运营月报（最新年份-上月最后一天日期范围）
-- 周大福_日期范围生成_yearly_cumulative_monthly
...
today() AS input_date), -- 可替换为任意日期
     ...
     DATE_RANGES_RAW_CTE AS (
         SELECT 'yearly_cumulative_monthly'                           AS report_type,
                ...
                subtractYears(input_year_start, 1)                    AS prev_start_date_raw, -- 去年1月1日
                subtractDays(subtractYears(input_month_start, 1), 1)  AS prev_end_date_raw    -- 去年上月最后一天
         FROM DATE_PARAM_CTE),
     DATE_RANGES_CTE AS (
         SELECT report_type,
                description,
