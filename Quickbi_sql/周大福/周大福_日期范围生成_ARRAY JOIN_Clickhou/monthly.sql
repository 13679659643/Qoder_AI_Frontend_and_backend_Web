-- ClickHouse 日期范围生成器 - 周大福运营月报（上月日期范围）
-- 周大福_日期范围生成_monthly
...
today() AS input_date), -- 可替换为任意日期
     ...
     DATE_RANGES_RAW_CTE AS (
         SELECT 'monthly'                                             AS report_type,
                '周大福运营月报（上月日期范围）'                        AS description,
                subtractMonths(input_month_start, 1)                  AS start_date_raw,      -- 上月第一天
                subtractDays(input_month_start, 1)                    AS end_date_raw,        -- 上月最后一天
                subtractYears(subtractMonths(input_month_start, 1), 1)  AS prev_start_date_raw, -- 去年同月第一天
                subtractDays(subtractYears(input_month_start, 1), 1)  AS prev_end_date_raw    -- 去年同月最后一天
         FROM DATE_PARAM_CTE),
     DATE_RANGES_CTE AS (
         SELECT report_type,
                description,
